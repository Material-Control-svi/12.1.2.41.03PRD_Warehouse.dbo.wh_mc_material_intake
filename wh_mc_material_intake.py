...
file.write("Start" + f" processing " + str(arrow.now().format('DD-MM-YYYY HH:mm:ss'))+"\n")
file.close()
...
for date_to_process in list_date_to_process :
    ...
    file = open(f'log/{project_name_log}.txt','a')
    file.write(f"Total time processing= {(time.time() - start_run_time)/ 60}"+"\n")
